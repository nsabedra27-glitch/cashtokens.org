...
While many use cases for numeric commitments can be emulated with only byte-string commitments, a numeric primitive enables many contracts to reduce or offload state management altogether (e.g. [shareholder voting](examples#voting-with-fungible-tokens)), simplifying contract audits and reducing transaction sizes.
...
With contract interoperability, behavior can be broken into clusters of smaller, coordinating contracts, **reducing transaction sizes**. This interoperability further enables [covenants](examples#usage-examples) to communicate over **public interfaces**, allowing diverse ecosystems of compatible covenants to work together, even when developed and deployed separately.
...
**Non-fungible tokens** are critical for coordinating activity trustlessly between multiple covenants, enabling [covenant-tracking tokens](examples#covenant-tracking-identity-tokens), [depository child covenants](examples#depository-child-covenants), [multithreaded covenants](examples#multithreaded-covenants), and other constructions in which a particular covenant instance must be authenticated.

**Fungible tokens** are valuable for covenants to represent on-chain assets – e.g. voting shares, utility tokens, collateralized loans, prediction market options, etc. – and implement [complex coordination tasks](examples#voting-with-fungible-tokens) – e.g. liquidity-pooling, auctions, voting, sidechain withdrawals, spin-offs, mergers, and more.
...
3. [Token inspection opcodes](#token-inspection-operations) allow contracts to operate on tokens, enabling [cross-contract interfaces and decentralized applications](examples#usage-examples).
...
1. By consensus, `commitment_length` is limited to `40` (`0x28`), but future upgrades may increase this limit. Implementers are advised to ensure that values between `253` (`0xfdfd00`) and `65535` (`0xfdffff`) can be parsed. (See [Non-Fungible Token Commitment Length](rationale#non-fungible-token-commitment-length).)
...
Several measurements of fungible token supply are standardized for wider ecosystem compatibility. (See [Specification of Token Supply Definitions](rationale#specification-of-token-supply-definitions).)
...
- [Appendix: Usage Examples &rarr;](examples#usage-examples)
  - [Identity Tokens](examples#identity-tokens)
  - [Covenant-Tracking Identity Tokens](examples#covenant-tracking-identity-tokens)
  - [Depository Child Covenants](examples#depository-child-covenants)
  - [Voting with Fungible Tokens](examples#voting-with-fungible-tokens)
    - [Sealed Voting](examples#sealed-voting)
  - [Multithreaded Covenants](examples#multithreaded-covenants)
  - [Multi-Covenant, Decentralized Applications](examples#multi-covenant-decentralized-applications)

## Rationale

- [Appendix: Rationale &rarr;](rationale#rationale)
  - [Incompatibility of Token Fungibility and Token Commitments](rationale#incompatibility-of-token-fungibility-and-token-commitments)
  - [Selection of Commitment Types](rationale#selection-of-commitment-types)
    - [Boolean Commitment Type](rationale#boolean-commitment-type)
    - [Bitfield Commitment Type](rationale#bitfield-commitment-type)
    - [Hash Commitment Types](rationale#hash-commitment-types)
      - [Secret Hash Commitment Type](rationale#secret-hash-commitment-type)
      - [Nonsecret Hash Commitment Type](rationale#nonsecret-hash-commitment-type)
    - [Public Key Commitment Type](rationale#public-key-commitment-type)
    - [Signature Commitment Type](rationale#signature-commitment-type)
  - [Shared Codepoint for All Tokens](rationale#shared-codepoint-for-all-tokens)
  - [Behavior of Minting and Mutable Tokens](rationale#behavior-of-minting-and-mutable-tokens)
  - [Exclusion of Cloneable Capability](rationale#exclusion-of-cloneable-capability)
  - [Avoiding Proof-of-Work for Token Data Compression](rationale#avoiding-proof-of-work-for-token-data-compression)
  - [Use of Transaction IDs as Token Category IDs](rationale#use-of-transaction-ids-as-token-category-ids)
  - [One Token Prefix Per Output](rationale#one-token-prefix-per-output)
  - [Including Capabilities in Token Category Inspection Operations](rationale#including-capabilities-in-token-category-inspection-operations)
  - [Support for Zero-Length Commitments](rationale#support-for-zero-length-commitments)
  - [Limitation of Non-Fungible Token Commitment Length](rationale#limitation-of-non-fungible-token-commitment-length)
  - [Inclusion of Token-Aware CashAddresses](rationale#inclusion-of-token-aware-cashaddresses)
  - [Recommendation of `SIGHASH_UTXOS` for Multi-Entity Transactions](rationale#recommendation-of-sighash_utxos-for-multi-entity-transactions)
  - [Limitation of Fungible Token Supply](rationale#limitation-of-fungible-token-supply)
  - [Specification of Token Supply Definitions](rationale#specification-of-token-supply-definitions)

## Prior Art & Alternatives

- [Appendix: Prior Art & Alternatives &rarr;](alternatives#prior-art--alternatives)
  - [PMv3](alternatives#pmv3)
  - [Bitauth](alternatives#bitauth)
  - [Colored Coins](alternatives#colored-coins)
    - [OP_CHECKCOLORVERIFY](alternatives#op_checkcolorverify)
    - [Freimarkets](alternatives#freimarkets)
    - [Confidential Assets](alternatives#confidential-assets)
    - [OP_GROUP](alternatives#op_group)
    - [Group Tokenization](alternatives#group-tokenization)
    - [Simple Ledger Protocol (v1)](alternatives#simple-ledger-protocol-v1)
    - [Unforgeable Groups](alternatives#unforgeable-groups)
